...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Ensure autoplay is enabled on load
  let src = iframe.getAttribute("src");
  if (!src.includes("autoplay=1")) {
    const separator = src.includes("?") ? "&" : "?";
    iframe.src = src + separator + "autoplay=1&muted=1";
  }
  ...
  iframe.addEventListener("load", function() {
    // Try to trigger autoplay by reloading with autoplay parameters
    setTimeout(() => {
      let currentSrc = iframe.src;
      if (!currentSrc.includes("autoplay=1")) {
        const separator = currentSrc.includes("?") ? "&" : "?";
        iframe.src = currentSrc + separator + "autoplay=1&muted=1";
      }
    }, 1000);
  });
  ...
  function pauseVideo() {
    if (isPlaying) {
      let src = iframe.getAttribute("src");
      iframe.src = src.replace(/[\?&]autoplay=1/g, "");
  ...
  iframe.addEventListener("click", pauseVideo);
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const equipmentData = {
    sirona: {
      title:
        "Системы Sirona позволяют внедрить полноценный цифровой цикл — от сканирования до готовой реставрации в клинике или лаборатории.",
      benefits: [
      ...
      conclusion:
        "Преимущества: скорость, предсказуемость результата, контроль качества на каждом этапе, экономия времени врача и пациента.",
    },
    halot: {
      title:
        "Линейка Halot решает задачи 3D-моделирования и аддитивного производства в стоматологии — от печати моделей челюстей до изготовления шаблонов и кап.",
      benefits: [
      ...
      conclusion:
        "Преимущества: чистая геометрия, стабильный процесс, низкая себестоимость печати и быстрая окупаемость оборудования.",
    },
    scanner: {
      title:
        "Интраоральные сканеры позволяют заменить традиционные слепки на точное цифровое 3D-сканирование полости рта",
      benefits: [
      ...
      conclusion:
        "Преимущества: скорость, комфорт, отсутствие ошибок, которые бывают при обычных слепках.",
    },
    "cad-cam": {
      title:
        "Современные CAD-решения упрощают планирование и проектирование ортопедических конструкций, повышая точность и скорость изготовления.",
      benefits: [
      ...
      conclusion:
        "Преимущества: прогнозируемый дизайн, меньше ручных корректировок, экономия времени врача и техника, быстрая подготовка к производству.",
    },
    sandblaster: {
      title:
        "Пескоструйные системы обеспечивают чистую и равномерную шероховатость поверхности для надежной фиксации облицовочных материалов.",
      benefits: [
      ...
      conclusion:
        "Преимущества: воспроизводимое качество поверхности, сокращение брака, долговечность соединения керамики с каркасом.",
    },
  };
  ...
  equipmentItems.forEach((item, index) => {
    item.addEventListener("click", function () {
      equipmentItems.forEach((el) =>
        el.classList.remove("equipment-item-active")
      );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function changeImageSet(direction) {
    if (direction === "left") {
      currentImageSet =
        (currentImageSet - 1 + imageSets.length) % imageSets.length;
